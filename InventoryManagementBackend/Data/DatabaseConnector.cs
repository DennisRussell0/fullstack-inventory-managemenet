// This class implements the IDatabaseConnector interface
// It provides methods to interact with the PostgreSQL database using Npgsql.
...
public class DatabaseConnector : IDatabaseConnector
{
    ...
    public List<Order> RetrieveOrders()
    {
        ...
        while (reader.Read())
        {
            List<ProductItem> products = RetrieveProductsByOrderId(reader.GetInt32(reader.GetOrdinal("id")));
    ...
    public bool PostOrder(Order newOrder)
    {
        ...
        {
            using var productCommand = new NpgsqlCommand("INSERT INTO order_products (orderid, productid, quantity) VALUES (@orderId, @productId, @quantity)", connection);
    ...
    public bool UpdateOrder(Order updatedOrder)
    {
        ...
        foreach (var product in updatedOrder.Products)
        {
            ...
            if (existingProduct != default)
            {
                ...
                using var updateCommand = new NpgsqlCommand("UPDATE order_products SET quantity = @quantity WHERE orderid = @orderId AND productid = @productId", connection);
            ...
            else
            {
                ...
                using var insertCommand = new NpgsqlCommand("INSERT INTO order_products (orderid, productid, quantity) VALUES (@orderId, @productId, @quantity)", connection);
        ...
        {
            if (!updatedOrder.Products.Any(p => p.ProductId == existingProduct.ProductId))
            {
                using var deleteCommand = new NpgsqlCommand("DELETE FROM order_products WHERE orderid = @orderId AND productid = @productId", connection);
                deleteCommand.Parameters.AddWithValue("@orderId", updatedOrder.Id);
                deleteCommand.Parameters.AddWithValue("@productId", existingProduct.ProductId);
    ...
    public bool DeleteOrder(int id)
    {
        ...
        using var deleteProductsCommand = new NpgsqlCommand("DELETE FROM order_products WHERE orderid = @id", connection);
    ...
    public List<ProductItem> RetrieveProductsByOrderId(int orderId)
    {
        ...
        using var command = new NpgsqlCommand("SELECT productid, quantity FROM order_products WHERE orderid = @orderId", connection);
        command.Parameters.AddWithValue("@orderId", orderId);

        using var reader = command.ExecuteReader();
        while (reader.Read())
        {
            products.Add(new ProductItem{ProductId = reader.GetInt32(0), Quantity = reader.GetInt32(1)});
        }
